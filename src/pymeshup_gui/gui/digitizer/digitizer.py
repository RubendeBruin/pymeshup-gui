...
class DigitizerDialog(QDialog):
    ...
    def apply_axis_limits(self):
        try:
            _x_min = float(self.x_min_edit.text())
            _x_max = float(self.x_max_edit.text())
            _y_min = float(self.y_min_edit.text())
            _y_max = float(self.y_max_edit.text())
        except ValueError:
    ...
    def paste_background_image(self):
        ...
        if mime_data.hasImage():
            ...
            if not qimage.isNull():
                ...
                # Print debug info
                print(
                    f"Image info - Format: {self._stored_qimage.format()}, Size: {self._stored_qimage.width()}x{self._stored_qimage.height()}"
                )
    ...
    def redraw_plot(self):
        try:
            ...
            print(
                "Invalid axis limits in text boxes. Using previous limits or default 0,10 for redraw."
            )
        ...
        if self._stored_qimage and not self._stored_qimage.isNull():
            try:
                ...
                if width > 0 and height > 0:
                    ...
                    qimage_rgba = self._stored_qimage.convertToFormat(
                    ...
                    if ptr:
                        ...
                        self.background_image_artist = self.ax.imshow(
                            img_data,
                            extent=[ax_xlim[0], ax_xlim[1], ax_ylim[0], ax_ylim[1]],  # type: ignore
